...
import { LabelComponent } from "@/app/components/Pages/Master/Raw/Print/LabelComponent";

// PrintAll component
export default function PrintAll({data}: {data: any}) {
  ...
  return (
    <>
      {
        modal && <div
          className="fixed bg-black h-screen bg-opacity-20 flex items-center justify-center top-0 left-0 z-[5000] w-full  overflow-x-hidden outline-none">
          <div className="w-2/4 h-2/3 rounded bg-white border-4 border-base">
            <div
            ...
            <div className="p-2 flex flex-col gap-5 h-full">
              <div ref={componentRef} className={`overflow-y-auto h-full text-black`}>
                {
                  data.map((labelData: any, index: number) => (
      ...
      <div>
        <div onClick={() => setModal(true)}
          ...
          <BiPrinter size={12}/>
          <p className={`text-white font-bold text-sm`}>Cetak Part Tag</p>
        </div>
      </div>
    </>
  )
